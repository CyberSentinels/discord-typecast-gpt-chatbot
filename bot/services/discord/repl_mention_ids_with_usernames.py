...
def repl_mention_ids_with_usernames(msg_content, msg_mentions):
    return re.sub(
        r"<@(\d+)>", lambda g: repl_id_with_username(g[1], msg_mentions), msg_content
    )
...
class TestReplMentionIdsWithUsernames:
    ...
    def test_no_mention_ids(self):
        ...
        msg_mentions = []
        assert repl_mention_ids_with_usernames(msg_content, msg_mentions) == msg_content
    ...
    def test_no_matching_mention_ids(self):
        ...
        assert (
            repl_mention_ids_with_usernames(msg_content, msg_mentions)
            == expected_output
        )
    ...
    def test_multiple_mention_ids(self):
        ...
        msg_mentions = [
            self.mention(id=1234, name="John"),
            self.mention(id=5678, name="Jane"),
        ]
